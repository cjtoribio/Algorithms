...
struct EulerTree {
    struct Node {
        ...
        void update(){
            sz = (gid & 1 ? 0 : 1) + (l?l->sz:0) + (r?r->sz:0);
    ...
    void print(bool asArray = false, function<void(Node*)> printFn = [](auto p) { cout << p->id; }){
        for(int i = 0; i < N; ++i)
            if(!ST[i]->p || !EN[i]->p){
                ...
                !asArray ? printTree(tp, printFn) : printArray(tp, printFn);
                if(!asArray)cout << "-----------------------" << endl;
                else cout << endl;
            }
    }
    void printTree(Node *v, const function<void(Node*)> printFn, int LVL = 0){
        if(v->r)printTree(v->r, printFn, LVL+1);
        cout << string(LVL,'\t') << (v->isEnd() ? "" : "(");
        printFn(v);
        cout << (v->isEnd() ? ")" : "") << endl;
        if(v->l)printTree(v->l, printFn, LVL+1);
    }
    void printArray(Node *v, const function<void(Node*)> printFn, int LVL = 0){
        v->pushDown();
        if(v->l)printArray(v->l, printFn, LVL + 1);
        printFn(v); cout << " ";
        if(v->r)printArray(v->r, printFn, LVL + 1);
    }
};
struct Int {
    int t = 0;
